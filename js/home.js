...
function lossGame(){
  ...
  svg.select("#state_indicator")
    .text("GAME OVER");
  $('#score_go').val(game_status.score);
      var highsco = localStorage.getItem('highscore');
	  var elem = $('#update');
	  if (game_status.score < highsco) {
	  $(elem ).addClass("pure-button-disabled");
	  }else {
	  $(elem ).removeClass("pure-button-disabled");
		}
	nav('gameover');
}
  ...
  var t = svg.select(".indicator").transition().ease("linear").duration(500)
    .attrTween("d", function(){
      ...
      if(end_score > threshold){
		...
		pattern_interval = pattern_interval - 2; 
		play(snd_multi);
...
function down(id){
      ...
      else if(id == 2)	{//press right
        control_status.rotate = -1;
      }
      else if(id == 0){//press space
